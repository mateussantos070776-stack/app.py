...
# 2. CSS MASTER (BORDA FIXA E ESTILO KERIGMA)
st.markdown("""
    <style>
    header {visibility: hidden;}
    .block-container { padding-top: 1rem !important; }
    
    /* MANTER BARRA LATERAL E BORDA SEMPRE VISÍVEIS */
    ...
    /* DESATIVAR BOTÃO DE RECOLHER PARA NÃO SUMIR A BORDA */
    ...
    /* CAIXA DE EXIBIÇÃO DE CREDENCIAIS NO ADM */
    .display-credencial {
...
if st.session_state.tela == "home":
    ...
    with col:
        ...
        if st.button("LOGAR NO SISTEMA", use_container_width=True):
            if user_input and chave_i:
                ...
                if user_input in usuarios and usuarios[user_input] == chave_i:
                    ...
                    st.error("Credenciais inválidas ou não autorizadas.")
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("<h3 style='text-align:center;'>🔑 Geração de Credenciais</h3>", unsafe_allow_html=True)
        st.markdown(f"""
        <div class="display-credencial">
            <p style="color:#888; margin:0;">ID GERADO</p>
        ...
        if st.button("GERAR NOVO ACESSO", use_container_width=True):
            # Gera ID (Letras e Números) e Chave (Só números)
    ...
    with c2:
        ...
        if st.button("VER LISTA DE USUÁRIOS", use_container_width=True):
            st.session_state.tela = "lista_usuarios"; st.rerun()

elif st.session_state.tela == "lista_usuarios":
    st.markdown("<h1 style='color:#E50914; text-align:center; font-weight:900;'>USUÁRIOS ATIVOS</h1>", unsafe_allow_html=True)
    ...
    st.markdown('<div style="border: 2px solid #E50914; border-radius: 15px; padding: 60px; text-align: center; background-color: #0a0a0a; margin-top: 100px;"><h1>ACESSO CONFIRMADO</h1><p>Sistema Kerigma - Mídia Maanaim</p></div>', unsafe_allow_html=True)

elif st.session_state.tela == "login_admin":
    st.markdown("<h1 style='color:#E50914; text-align:center; font-weight:900;'>LOGIN ADM</h1>", unsafe_allow_html=True)
