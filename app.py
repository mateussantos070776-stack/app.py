...
# 2. CSS MASTER (TRAVAMENTO TOTAL DA BORDA E DESIGN)
st.markdown("""
    <style>
    header {visibility: hidden;}
    .block-container { padding-top: 0rem !important; }
    
    /* PROTEÇÃO DA BARRA LATERAL */
...
if st.session_state.tela == "home":
    ...
    with col:
        user_input = st.text_input("ID DE USUÁRIO").strip().upper()
        ...
        if st.button("LOGAR", use_container_width=True):
            ...
            if user_input in usuarios and usuarios[user_input] == chave_i:
                ...
                st.error("Credenciais inválidas. Solicite ao seu ADM.")
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("<h3 style='text-align:center;'>🔑 Gerar Credencial</h3>", unsafe_allow_html=True)
        st.markdown(f"""
        <div class="card-gerador">
            <p style="color:#888; margin:0;">ID DE USUÁRIO</p>
        ...
        if st.button("GERAR NOVO ACESSO", use_container_width=True):
            ...
            st.rerun()
    with c2:
        st.markdown("<h3 style='text-align:center;'>👥 Membros Ativos</h3>", unsafe_allow_html=True)
        if st.button("GERENCIAR LISTA", use_container_width=True):
            st.session_state.tela = "lista_usuarios"; st.rerun()

elif st.session_state.tela == "lista_usuarios":
    st.markdown("<h1 style='color:#E50914; text-align:center; font-weight:900;'>LISTA DE ACESSOS</h1>", unsafe_allow_html=True)
    ...
    st.markdown('<div style="border: 2px solid #E50914; border-radius: 15px; padding: 80px; text-align: center; background-color: #0a0a0a; margin-top: 100px;"><h1>ACESSO AUTORIZADO</h1><p>Sistema Kerigma - Mídia Maanaim</p></div>', unsafe_allow_html=True)

elif st.session_state.tela == "login_admin":
    st.markdown("<h1 style='color:#E50914; text-align:center; font-weight:900;'>ADMINISTRAÇÃO</h1>", unsafe_allow_html=True)
