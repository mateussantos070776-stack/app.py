...
# 2. CSS ORIGINAL (BARRA LATERAL COM BORDA VERMELHA)
st.markdown("""
    <style>
    header {visibility: hidden;}
    .block-container { padding-top: 0rem !important; }
    
    /* ESTILO DA SIDEBAR ORIGINAL */
    ...
    /* OCULTAR BOTÕES DE RECOLHER PARA PRESERVAR O LAYOUT */
...
if st.session_state.tela == "home":
    ...
    st.markdown("<h1 style='color:#E50914; text-align:center; font-weight:900;'>REGISTRO / LOGIN</h1>", unsafe_allow_html=True)
    ...
    with col:
        ...
        if st.button("ENTRAR", use_container_width=True):
            if nome_i and chave_i:
                ...
                # Se o nome já existe e a chave bate, loga
                ...
                # Se a chave não foi usada por ninguém ainda, registra o novo nome
                elif chave_i not in registrados.values() and nome_i not in registrados:
                    salvar_usuario_no_arquivo(nome_i, chave_i)
                    st.session_state.autenticado = True; st.session_state.tela = "painel_membro"; st.rerun()
                else:
                    st.error("Chave inválida ou já utilizada por outro membro.")
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("<h3 style='text-align:center;'>🔑 Gerar Chave</h3>", unsafe_allow_html=True)
    ...
    st.markdown("<h1 style='color:#E50914; text-align:center; font-weight:900;'>USUÁRIOS CADASTRADOS</h1>", unsafe_allow_html=True)
    ...
    st.markdown('<div style="border: 2px solid #E50914; border-radius: 15px; padding: 60px; text-align: center; background-color: #0a0a0a; margin-top: 100px;"><h1 style="color:#E50914; font-size:40px; font-weight:900;">ACESSO LIBERADO</h1><p>Bem-vindo à área de membros da Mídia Maanaim.</p></div>', unsafe_allow_html=True)
