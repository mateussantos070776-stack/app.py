...
# 2. CSS MASTER (BARRA LATERAL FIXA E VISÍVEL)
st.markdown("""
    <style>
    header {visibility: hidden;}
    .block-container { padding-top: 1rem !important; }
    
    /* FORÇAR EXIBIÇÃO DA SIDEBAR NO PC */
    ...
    /* OCULTAR BOTÕES NATIVOS DE RECOLHER */
...
if st.session_state.tela == "home":
    ...
    with col:
        ...
        if st.button("ENTRAR", use_container_width=True):
            ...
            if user_input in reg and reg[user_input] == chave_i:
                ...
                st.error("Credenciais incorretas ou não geradas pelo ADM.")
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("<h3 style='text-align:center;'>🔑 Gerar Acesso</h3>", unsafe_allow_html=True)
        st.markdown(f"""
        ...
        if st.button("GERAR NOVO ID + CHAVE", use_container_width=True):
    ...
    st.markdown('<div class="janela-desenvolvimento"><h1>SISTEMA ATIVO</h1><p>Acesso concedido à Equipe Mídia.</p></div>', unsafe_allow_html=True)

elif st.session_state.tela == "login_admin":
    st.markdown("<h1 style='color:#E50914; text-align:center; font-weight:900;'>ADM LOGIN</h1>", unsafe_allow_html=True)
